        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('pengajuan-saya.update', $requestLetter->id) }}" method="POST"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="request_type_id" id="request_type_id"
                                    class="form-control @error('request_type_id') is-invalid @enderror" required>
                                    @if (isset($requestLetter))
                                        <option value="{{ $requestLetter->request_type_id }}" selected
                                            data-code="{{ $requestLetter->requestType->code }}"
                                            data-fields="{{ $requestLetter->requestType->additional_fields }}"
                                            data-documents="{{ $requestLetter->requestType->required_documents }}">
                                    ...
                                    @else
                                        ...
                                        @foreach ($requestTypes as $type)
                                            <option value="{{ $type->id }}" data-code="{{ $type->code }}"
                                                data-fields="{{ $type->additional_fields }}"
                                                data-documents="{{ $type->required_documents }}"
