                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ...
                            @if ($requestLetter && $requestLetter->status == 'Diajukan')
                                <a href="{{ route('pengajuan-saya.edit', $requestLetter->id) }}"
                                    class="btn btn-sm btn-warning">
                                    <i class="fas fa-edit"></i> Edit
                                </a>
                                <form action="{{ route('pengajuan-saya.destroy', $requestLetter->id) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @elseif ($requestLetter && $requestLetter->status == 'Selesai')
                                <a href="{{ route('pengajuan-saya.print', $requestLetter->id) }}" target="_blank"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="card shadow-sm mb-4">
                                        <div class="card-body">
                                            ...
                                            @php
                                                $requestData = json_decode($requestLetter->data);
                                            @endphp

                                            <div class="row mb-3">
                                                <div class="col-md-5 fw-bold">NIK</div>
                                                <div class="col-md-7">
                                                    {{ $requestLetter->user?->resident?->nik ?? ($requestData->nik ?? '-') }}
                                                </div>
                                            </div>
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-md-7">
                                                    {{ $requestLetter->user?->resident?->address ?? ($requestData->address ?? '-') }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    @else
                                                        <div class="row mb-3">
                                                            <div class="col-md-5 fw-bold">
                                                                {{ __('request-letter.' . $key) }}</div>
                                                            <div class="col-md-7">
                                                                {{ str_contains($key, 'income') ? number_format($value) : $value }}
                                                            </div>
                                                        </div>
                                                    @endif
                                                @endif
                                                ...
                                                @foreach ($requestLetter->historyRequestLetters as $history)
                                                    <div class="timeline-item mb-4">
                                                        <div class="d-flex align-items-center mb-2">
                                                            <span
                                                                class="badge bg-{{ $history->status == 'Diajukan' ? 'warning' : ($history->status == 'Diproses' ? 'info' : ($history->status == 'Selesai' ? 'success' : 'danger')) }} me-2">
                                                                {{ $history->status }}
                                                            </span>
                                                            <small
